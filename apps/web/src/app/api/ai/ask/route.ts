...
Response:`,
        ...
        if (error instanceof Error) {
            if (error.message === 'Too many AI requests') {
                return NextResponse.json(
                    { message: 'Daily AI request limit of 300 requests reached. Please try again tomorrow.', status: 429 },
                    {
                        status: 429,
                        headers: {
        ...
        return NextResponse.json(
            { message: 'An error occurred while processing your request', status: 500 },
            { status: 500 }
        )
    }
}
